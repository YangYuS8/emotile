...
function putPixel(pixels: Pixel[], x: number, y: number, color: PixelColor): void {
  ...
  for (let dy = -r; dy <= r; dy++) {
    for (let dx = -r; dx <= r; dx++) {
      if (dx * dx + dy * dy <= r * r + r) {
        ...
        if (dy <= threshold) {
          putPixel(pixels, Math.round(eye.x) + dx, Math.round(eye.y) + dy, "primary");
        }
      }
    }
  }
...
function renderFlatMouth(pixels: Pixel[], mouth: Mouth): void {
  ...
  putHLine(pixels, mouth.x - half, mouth.x + half, Math.round(mouth.y), "primary");
}
...
function renderSmileMouth(pixels: Pixel[], mouth: Mouth): void {
  ...
  putHLine(pixels, mouth.x - half, mouth.x + half, Math.round(mouth.y), "primary");
  // curve down
  const depth = Math.max(1, Math.round(Math.abs(mouth.curve) * mouth.width * 0.3));
  for (let d = 1; d <= depth; d++) {
    const shrink = Math.round((d / depth) * half * 0.8);
    putHLine(pixels, mouth.x - half + shrink, mouth.x + half - shrink, Math.round(mouth.y) + d, "primary");
  }
}
...
function renderOpenMouth(pixels: Pixel[], mouth: Mouth): void {
  ...
  for (let dy = 0; dy <= depth; dy++) {
    ...
    putHLine(pixels, mouth.x - half + shrink, mouth.x + half - shrink, Math.round(mouth.y) + dy, "primary");
  }
}
...
function renderBrokenMouth(pixels: Pixel[], mouth: Mouth): void {
  ...
  putHLine(pixels, mouth.x + 1, mouth.x + half, Math.round(mouth.y) + 1, "primary");
  // wobble
  const depth = Math.max(1, Math.round(Math.abs(mouth.curve) * mouth.width * 0.15));
  for (let d = 1; d <= depth; d++) {
...
const MOUTH_RENDERERS: Record<MouthShape, (pixels: Pixel[], mouth: Mouth) => void> = {
...
function renderSmokeMark(pixels: Pixel[], mark: Mark): void {
  for (let dy = 0; dy < 3; dy++) {
    ...
    putPixel(pixels, Math.round(mark.x) + offset, Math.round(mark.y) - dy, "shadow");
  }
}
...
function renderAngerMark(pixels: Pixel[], mark: Mark): void {
  ...
  for (let d = 0; d < 3; d++) {
    putPixel(pixels, Math.round(mark.x) + d, Math.round(mark.y) - d + 2, "shadow");
  }
}
...
export function renderExpression(expression: EmotileExpression): PixelFrame {
  ...
  const rightRenderer = EYE_RENDERERS[expression.eyes.right.shape] ?? renderDotEye;
  rightRenderer(pixels, expression.eyes.right);

  // Render mouth
  const mouthRenderer = MOUTH_RENDERERS[expression.mouth.shape] ?? renderFlatMouth;
