...
**v0.4 Release Candidate** —— 发布自动化、集成准备，以及完整的 v0.3 表达式表面。以纯数据形式完成校验、归一化、修复、渲染、主题化、SVG 导出，并提供轻量 CLI。不依赖浏览器、Canvas 或 GPU。
...
- **v0.3**: 外部主题/调色板运行时、SVG 渲染器、JSON Schema 导出、轻量 CLI、图库示例。
- **v0.4**（当前）: 发布自动化、集成准备指南、Agent 集成文档。
- **v1.0**: 稳定规范、完整动画运行时。
